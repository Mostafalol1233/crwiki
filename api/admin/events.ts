...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (!upstream.ok) return addCorsHeaders(res).status(upstream.status).json({ error: `Supabase ${table} query failed`, details: payload });
    ...
    if (req.method === "POST") {
      ...
      if (!upstream.ok) return addCorsHeaders(res).status(upstream.status).json({ error: `Supabase ${table} insert failed`, details: payload });
    ...
    if (!id) return addCorsHeaders(res).status(400).json({ error: `${table === "events" ? "Event" : "Announcement"} id is required` });
    ...
    if (req.method === "PATCH") {
      ...
      const payload = await upstream.json().catch(() => null);
      if (!upstream.ok) return addCorsHeaders(res).status(upstream.status).json({ error: `Supabase ${table} update failed`, details: payload });
      return addCorsHeaders(res).status(200).json({ data: Array.isArray(payload) ? payload[0] || null : payload, error: null });
    }
    ...
    if (req.method === "DELETE") {
      ...
      if (!upstream.ok) return addCorsHeaders(res).status(upstream.status).json({ error: `Supabase ${table} delete failed`, details: await upstream.text() });
