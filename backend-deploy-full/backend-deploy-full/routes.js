...
const MIRRORED_DIR = path.resolve('backend-deploy-full/uploads/mirrored');
ensureDir(IMAGES_DIR);
ensureDir(BACKUP_DIR);
ensureDir(MIRRORED_DIR);
const CLOUDINARY_CLOUD_NAME = String(process.env.CLOUDINARY_CLOUD_NAME || '').trim();
const CLOUDINARY_BASE_URL = String(process.env.CLOUDINARY_BASE_URL || process.env.CLOUDINARY_DELIVERY_URL || (CLOUDINARY_CLOUD_NAME ? `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}` : 'https://res.cloudinary.com')).replace(/\/+$/, '');
const CLOUDINARY_HOST = (() => {
    try {
        return new URL(CLOUDINARY_BASE_URL).hostname;
    } catch {
        return 'res.cloudinary.com';
    }
})();
const buildCloudinaryUrl = (assetPath) => `${CLOUDINARY_BASE_URL}/${String(assetPath || '').replace(/^\/+/, '')}`;
...
export async function registerRoutes(app) {
    ...
    const resolveCloudinaryName = () => {
        const direct = String(process.env.CLOUDINARY_CLOUD_NAME || '').trim();
        if (direct) return direct;
        const base = String(process.env.CLOUDINARY_BASE_URL || '').trim();
        if (base) {
            try {
    ...
    const resolveCloudinaryBaseUrl = () => {
        const base = String(process.env.CLOUDINARY_BASE_URL || '').trim().replace(/\/$/, '');
        if (base) return base;
        const name = resolveCloudinaryName();
        if (name) return `https://res.cloudinary.com/${name}`;
        return 'https://res.cloudinary.com';
    };
    const isCloudinaryUrl = (url) => {
        try {
            const u = new URL(String(url));
            const base = resolveCloudinaryBaseUrl();
    ...
    const resolveAbsoluteUrl = async (url, req) => {
    ...
    const readLocalBuffer = async (relativePath) => {
        const normalized = String(relativePath || '').replace(/\\/g, '/');
        if (!normalized.startsWith('/')) return null;
        const clean = normalized.replace(/\/+/g, '/');
        if (clean.startsWith('/uploads/mirrored/')) {
            const fileRel = clean.replace('/uploads/mirrored/', '');
            const filePath = path.join(MIRRORED_DIR, fileRel);
            if (fs.existsSync(filePath)) return fs.promises.readFile(filePath);
        }
        if (clean.startsWith('/images/')) {
            const fileRel = clean.replace('/images/', '');
            const filePath = path.join(IMAGES_DIR, fileRel);
            if (fs.existsSync(filePath)) return fs.promises.readFile(filePath);
        }
        return null;
    };
    ...
    const resolveBufferFromUrl = async (url, req) => {
        ...
        if (src.startsWith('/')) {
            const local = await readLocalBuffer(src);
            if (local) {
                const ext = path.extname(src);
                return { buffer: local, contentType: extToMime(ext) };
            }
            const abs = await resolveAbsoluteUrl(src, req);
            return fetchRemoteBuffer(abs);
        }
        if (src.startsWith('http')) {
            ...
            if (u.hostname === baseHost) {
                const local = await readLocalBuffer(u.pathname);
                if (local) {
                    const ext = path.extname(u.pathname);
                    return { buffer: local, contentType: extToMime(ext) };
                }
            }
            return fetchRemoteBuffer(src);
        }
        return null;
    };
    const uploadBufferToCloudinary = async (buffer, filename, mimetype, folder, req) => {
        const json = await cloudinarySignedUpload(buffer, filename, mimetype, { folder });
        const secureUrl = json.secure_url;
        const domainUrl = await buildDomainUrl(secureUrl, req);
        return { secureUrl, domainUrl, publicId: json.public_id };
    };
    const cacheImageToCloudinary = async (url, { folder, filenameHint }, req) => {
        const src = String(url || '').trim();
        if (!src) return null;
        if (isCloudinaryUrl(src)) return { secureUrl: src, domainUrl: src, publicId: '' };
        const resolved = await resolveBufferFromUrl(src, req);
        if (!resolved || !resolved.buffer) return null;
        const ext = mimeToExt(resolved.contentType || '');
        const safeName = sanitizeFilename(filenameHint || 'image');
        const filename = `${safeName || 'image'}.${ext || 'jpg'}`;
        return uploadBufferToCloudinary(resolved.buffer, filename, resolved.contentType || extToMime(ext), folder, req);
    };
    ...
    app.get('/uploads/*', async (req, res) => {
        try {
            const cloudBase = resolveCloudinaryBaseUrl();
            const assetPath = req.params[0] || req.path.replace(/^\/uploads\//, '');
            const cloudinaryUrl = `${cloudBase}/image/upload/${assetPath}`;
    ...
    app.post("/api/posts", requireAuth, requirePostManager, upload.single('image'), async (req, res) => {
        try {
            let raw = normalizeContentPayload(req.body);
            let data = insertPostSchema.parse(raw);
            if (req.file) {
            ...
            const canonical = `${baseUrl}/article/${unique}`;
            data.post_slug = unique;
            data.canonicalUrl = data.canonicalUrl || canonical;
            data = applySeoDefaults(data);
            data = await cacheMediaFields(data, { folder: 'posts', name: data.title }, req);
    ...
    app.patch("/api/posts/:id", requireAuth, requirePostManager, async (req, res) => {
        try {
            let updates = normalizeContentPayload(req.body);
            ...
            if (updates.post_slug) {
                const allResult = await storage.getAllPosts().catch(() => ({ items: [] }));
                const all = allResult.items || [];
                const unique = ensureUniqueSlug(all.filter((p) => p.id !== req.params.id), 'post_slug', updates.post_slug);
                updates.post_slug = unique;
                const baseUrl = await resolveBaseUrl();
                updates.canonicalUrl = updates.canonicalUrl || `${baseUrl}/article/${unique}`;
            }
            updates = applySeoDefaults(updates);
            updates = await cacheMediaFields(updates, { folder: 'posts', name: updates.title || 'post' }, req);
    ...
    app.get('/api/posts/:id/seo', requireAuth, requirePostManager, async (req, res) => {
        try {
            ...
            res.json({
                ...
                twitterImageAlt: p.twitterImageAlt || '',
    ...
    app.put('/api/posts/:id/seo', requireAuth, requirePostManager, async (req, res) => {
        try {
            ...
            if (body.ogImageAlt !== undefined) updates.ogImageAlt = body.ogImageAlt;
            if (body.ogType !== undefined) updates.ogType = body.ogType;
            if (body.ogUrl !== undefined) updates.ogUrl = body.ogUrl;
            if (body.twitterTitle !== undefined) updates.twitterTitle = body.twitterTitle;
            if (body.twitterDescription !== undefined) updates.twitterDescription = body.twitterDescription;
            if (body.twitterImage !== undefined) updates.twitterImage = body.twitterImage;
            if (body.twitterImageAlt !== undefined) updates.twitterImageAlt = body.twitterImageAlt;
            if (body.schemaType !== undefined) updates.schemaType = body.schemaType;
            if (body.schemaData !== undefined) updates.schemaData = body.schemaData;

            const postBefore = await storage.getPostById(req.params.id);
            if (!postBefore) return res.status(404).json({ ok: false, error: 'Post not found' });
            updates = normalizeContentPayload(updates);
            updates = applySeoDefaults(updates);
            updates = await cacheMediaFields(updates, { folder: 'posts', name: postBefore.title || 'post' }, req);
            const post = await storage.updatePost(req.params.id, updates);
    ...
    app.get("/api/events", async (req, res) => {
        try {
            ...
            res.json({
                items: result.items.map((event) => ({ ...event, image: event.imageUrl || event.image || '' })),
                total: result.total
            });
        }
        catch (error) {
    ...
    app.get("/api/events/:id", async (req, res) => {
        try {
            ...
            res.json({ ...event, image: event.imageUrl || event.image || '' });
        }
        catch (error) {
            res.status(500).json({ error: error.message });
        }
    ...
    const normalizeContentPayload = (payload) => {
        ...
        if (out.imageUrl && !out.image) out.image = out.imageUrl;
        if (typeof out.seoKeywords === 'string') {
            out.seoKeywords = out.seoKeywords.split(',').map((s) => s.trim()).filter(Boolean);
        }
        return out;
    };
    const applySeoDefaults = (payload) => {
        const out = { ...(payload || {}) };
        if (!out.ogTitle && out.seoTitle) out.ogTitle = out.seoTitle;
        if (!out.ogDescription && out.seoDescription) out.ogDescription = out.seoDescription;
        if (!out.twitterTitle && out.ogTitle) out.twitterTitle = out.ogTitle;
        if (!out.twitterDescription && out.ogDescription) out.twitterDescription = out.ogDescription;
        if (!out.ogImageAlt && out.imageAlt) out.ogImageAlt = out.imageAlt;
        if (!out.twitterImageAlt && out.imageAlt) out.twitterImageAlt = out.imageAlt;
        if (!out.ogUrl && out.canonicalUrl) out.ogUrl = out.canonicalUrl;
        if (!out.ogType && out.schemaType) out.ogType = out.schemaType.toLowerCase().includes('event') ? 'event' : 'article';
        return out;
    };
    const cacheMediaFields = async (payload, { folder, name }, req) => {
        const out = { ...(payload || {}) };
        const baseName = sanitizeFilename(name || 'image') || 'image';
        const mapOne = async (src, suffix) => {
            const cached = await cacheImageToCloudinary(src, { folder, filenameHint: `${baseName}-${suffix}` }, req);
            if (cached?.secureUrl) return cached.secureUrl;
            return src;
        };
        if (out.imageUrl) {
            out.imageUrl = await mapOne(out.imageUrl, 'hero');
            out.image = out.imageUrl;
        }
        if (out.ogImage) out.ogImage = await mapOne(out.ogImage, 'og');
        if (out.twitterImage) out.twitterImage = await mapOne(out.twitterImage, 'twitter');
        if (Array.isArray(out.images)) {
            const nextImages = [];
            for (const [idx, img] of out.images.entries()) {
                const cached = await cacheImageToCloudinary(img, { folder, filenameHint: `${baseName}-gallery-${idx + 1}` }, req);
                nextImages.push(cached?.secureUrl || img);
            }
            out.images = nextImages;
        }
        return out;
    };
    app.post("/api/events", requireAuth, requireEventManager, upload.single('image'), async (req, res) => {
        try {
            let raw = normalizeContentPayload(req.body);
            let data = insertEventSchema.parse(raw);
            if (req.file) {
            ...
            const canonical = `${baseUrl}/events/${unique}`;
            data.event_name_slug = unique;
            data.canonicalUrl = data.canonicalUrl || canonical;
            data = applySeoDefaults(data);
            data = await cacheMediaFields(data, { folder: 'events', name: data.title }, req);
            const event = await storage.createEvent({ ...data, event_name_slug: unique, canonicalUrl: data.canonicalUrl });
    ...
    app.patch("/api/events/:id", requireAuth, requireEventManager, async (req, res) => {
        try {
            ...
            if (updates.event_name_slug) {
                const allResult = await storage.getAllEvents().catch(() => ({ items: [] }));
                const all = allResult.items || [];
                const unique = ensureUniqueSlug(all.filter((e) => e.id !== req.params.id), 'event_name_slug', updates.event_name_slug);
                updates.event_name_slug = unique;
                const baseUrl = await resolveBaseUrl();
                updates.canonicalUrl = updates.canonicalUrl || `${baseUrl}/events/${unique}`;
            }
            updates = applySeoDefaults(updates);
            updates = await cacheMediaFields(updates, { folder: 'events', name: updates.title || 'event' }, req);
    ...
    app.get('/api/events/:id/seo', requireAuth, requireEventManager, async (req, res) => {
        try {
            ...
            res.json({
                ...
                twitterImageAlt: e.twitterImageAlt || '',
    ...
    app.put('/api/events/:id/seo', requireAuth, requireEventManager, async (req, res) => {
        try {
            ...
            const eventBefore = await storage.getEventById(req.params.id);
            if (!eventBefore) return res.status(404).json({ ok: false, error: 'Event not found' });
            updates = normalizeContentPayload(updates);
            updates = applySeoDefaults(updates);
            updates = await cacheMediaFields(updates, { folder: 'events', name: eventBefore.title || 'event' }, req);
            const event = await storage.updateEvent(req.params.id, updates);
            ...
            for (const post of postsToCreate) {
                try {
                    ...
                    const eventData = {
                        ...
                        type: 'upcoming',
                        imageUrl: details.image || 'https://files.catbox.moe/wof38b.jpeg',
                        imageAlt: details.imageAlt || '',
                        seoDescription: details.preview || summarize(title),
                    };
                    eventData.image = eventData.imageUrl;
                    ...
                    if (!eventData.image || eventData.image.includes('wof38b')) {
                        try {
                            ...
                            if (seoImage?.url) {
                                eventData.image = seoImage.url;
                                eventData.imageUrl = seoImage.url;
                            }
                        } catch { }
                    }

                    let validated = insertEventSchema.parse(eventData);
                    const allResult = await storage.getAllEvents().catch(() => ({ items: [] }));
                    const all = allResult.items || [];
                    const baseUrl = await resolveBaseUrl();
                    const baseSlug = validated.event_name_slug || validated.title || '';
                    const unique = ensureUniqueSlug(all, 'event_name_slug', baseSlug);
                    const canonical = `${baseUrl}/events/${unique}`;
                    validated.event_name_slug = unique;
                    validated.canonicalUrl = validated.canonicalUrl || canonical;
                    validated = applySeoDefaults(validated);
...
`;
    ...
    app.get("/api/maps", async (req, res) => {
        try {
            ...
            res.json(maps.map((m) => ({ ...m, image: m.imageUrl || m.image || '' })));
        }
        catch (error) {
            res.status(500).json({ error: error.message });
        }
    ...
    app.get("/api/maps/:id", async (req, res) => {
        try {
            const map = await storage.getMapById(req.params.id);
            if (!map) {
                return res.status(404).json({ error: "Map not found" });
            }
            res.json({ ...map, image: map.imageUrl || map.image || '' });
        }
        catch (error) {
            res.status(500).json({ error: error.message });
        }
    ...
    app.post("/api/admin/scrape-and-create-maps", requireAuth, requireSuperAdmin, async (req, res) => {
        try {
            ...
            for (const m of maps) {
                ...
                const imageCandidate = m.imageUrl || m.image || '';
                const isGood = await isValidImage(imageCandidate);
                ...
                if (!exists) {
                    const data = insertMapSchema.parse({
                        name: m.name,
                        map_slug: m.map_slug,
                        imageUrl: isGood ? imageCandidate : "",
                        imageHistory: isGood ? [{ url: imageCandidate }] : [],
                        thumbnailUrl: m.thumbnailUrl || '',
                        minimapUrl: m.minimapUrl || '',
                        description: m.description || '',
                        loreDescription: m.loreDescription || '',
                        releaseDate: m.releaseDate || '',
                        designer: m.designer || '',
                        modeCompatibility: m.modeCompatibility || [],
                        mode: m.mode || '',
                    ...
                    const updateData = {};
                    if (isGood && imageCandidate !== exists.imageUrl) {
                        updateData.imageUrl = imageCandidate;
                        updateData.imageHistory = [...(exists.imageHistory || []), { url: imageCandidate }];
                    }
                    if (m.map_slug) updateData.map_slug = m.map_slug;
                    if (m.thumbnailUrl) updateData.thumbnailUrl = m.thumbnailUrl;
                    if (m.minimapUrl) updateData.minimapUrl = m.minimapUrl;
                    if (m.description !== undefined) updateData.description = m.description;
                    if (m.loreDescription !== undefined) updateData.loreDescription = m.loreDescription;
                    if (m.releaseDate !== undefined) updateData.releaseDate = m.releaseDate;
                    if (m.designer !== undefined) updateData.designer = m.designer;
                    if (m.modeCompatibility) updateData.modeCompatibility = m.modeCompatibility;
                    if (m.mode) updateData.mode = m.mode;
                    if (m.category) updateData.category = m.category;
                    if (m.sourceUrl) updateData.sourceUrl = m.sourceUrl;
    ...
    app.post("/api/events/bulk-create", requireAuth, requireEventScraper, async (req, res) => {
        ...
        app.get("/api/news", async (req, res) => {
            try {
                ...
                res.json({
                items: result.items.map((news) => ({ ...news, image: news.imageUrl || news.image || '' })),
                    total: result.total
                });
            }
            catch (error) {
        ...
        app.get("/api/news/:id", async (req, res) => {
            ...
            res.json({ ...news, image: news.imageUrl || news.image || '' });
            }
            catch (error) {
                res.status(500).json({ error: error.message });
            }
        ...
        app.post("/api/news", requireAuth, requireNewsManager, upload.single('image'), async (req, res) => {
            try {
                let raw = normalizeContentPayload(req.body);
                let data = insertNewsSchema.parse(raw);
                if (req.file) {
                ...
                const canonical = `${baseUrl}/news/${unique}`;
                data.news_slug = unique;
                data.canonicalUrl = data.canonicalUrl || canonical;
                data = applySeoDefaults(data);
                data = await cacheMediaFields(data, { folder: 'news', name: data.title }, req);
                const news = await storage.createNews({ ...data, news_slug: unique, canonicalUrl: data.canonicalUrl });
        ...
        app.patch("/api/news/:id", requireAuth, requireNewsManager, async (req, res) => {
            try {
                ...
                if (updates.news_slug) {
                    const allResult = await storage.getAllNews().catch(() => ({ items: [] }));
                    const all = allResult.items || [];
                    const unique = ensureUniqueSlug(all.filter((n) => n.id !== req.params.id), 'news_slug', updates.news_slug);
                    updates.news_slug = unique;
                    const baseUrl = await resolveBaseUrl();
                    updates.canonicalUrl = updates.canonicalUrl || `${baseUrl}/news/${unique}`;
                }
                updates = applySeoDefaults(updates);
                updates = await cacheMediaFields(updates, { folder: 'news', name: updates.title || 'news' }, req);
        ...
        app.get('/api/news/:id/seo', requireAuth, requireNewsManager, async (req, res) => {
            try {
                ...
                res.json({
                    ...
                    ogImage: n.ogImage || '',
                    ogImageAlt: n.ogImageAlt || '',
                    ogType: n.ogType || '',
                    ogUrl: n.ogUrl || '',
                    twitterTitle: n.twitterTitle || '',
                    twitterDescription: n.twitterDescription || '',
                    twitterImage: n.twitterImage || '',
                    twitterImageAlt: n.twitterImageAlt || '',
                    schemaType: n.schemaType || 'NewsArticle',
                    schemaData: n.schemaData || {},
                });
            } catch (error) {
                res.status(500).json({ ok: false, error: error.message });
            }
        ...
        app.put('/api/news/:id/seo', requireAuth, requireNewsManager, async (req, res) => {
            try {
                ...
                let updates = {
                    ...
                    twitterImageAlt: body.twitterImageAlt,
                    schemaType: body.schemaType || 'NewsArticle',
                    schemaData: body.schemaData,
                };
                const newsBefore = await storage.getNewsById(req.params.id);
                if (!newsBefore) return res.status(404).json({ ok: false, error: 'News item not found' });
                updates = normalizeContentPayload(updates);
                updates = applySeoDefaults(updates);
                updates = await cacheMediaFields(updates, { folder: 'news', name: newsBefore.title || 'news' }, req);
        ...
        app.post("/api/upload-video", uploadLimiter, requireAuth, upload.single('video'), async (req, res) => {
            try {
                ...
                try {
                    ...
                    const secureUrl = json.secure_url;
                    const domainUrl = await buildDomainUrl(secureUrl, req);
                    const CLOUDINARY_CLOUD_NAME = resolveCloudinaryName();
                    let thumbnail_secure_url = '';
                    let thumbnail_domain_url = '';
                    try {
                        const pub = String(json.public_id || '').trim();
                        if (pub) {
                            thumbnail_secure_url = `${resolveCloudinaryBaseUrl()}/video/upload/so_0/${pub}.jpg`;
                    ...
                    const secureUrl = `${resolveCloudinaryBaseUrl()}/${parts.join('/')}`;
        ...
        async function buildDomainUrl(secureUrl, req) {
            try {
                ...
                const cloudBase = resolveCloudinaryBaseUrl();
                const cloudHost = new URL(cloudBase).hostname;
                if (u.hostname !== cloudHost) return String(secureUrl);
                const parts = u.pathname.split('/').filter(Boolean);
        ...
        async function cloudinarySignedUpload(buffer, filename, mimetype, opts = {}) {
            ...
            if (DRY_RUN) {
                ...
                const base = resolveCloudinaryBaseUrl();
                return {
                    secure_url: `${base}/${kind}/upload/v123/${publicIdBase}.${format}`,
        ...
        app.post('/images/upload', uploadLimiter, upload.single('file'), async (req, res) => {
            try {
                ...
                try {
                    ...
                    const secureUrl = `${resolveCloudinaryBaseUrl()}/${localPathParts.join('/')}`;
        ...
        app.get('/cdn/fetch', async (req, res) => {
            try {
                ...
                const u = new URL(url);
                const cloudHost = new URL(resolveCloudinaryBaseUrl()).hostname;
                if (u.hostname !== cloudHost) {
        ...
        app.all('/media/cloudinary/*', async (req, res) => {
            try {
                ...
                const cloudName = resolveCloudinaryName();
                if (!rest || (cloudName && !rest.startsWith(`${cloudName}/`))) {
                    return res.status(400).json({ ok: false, error: 'Invalid Cloudinary path' });
                }
                const url = `${resolveCloudinaryBaseUrl()}/${rest}`;
                const u = new URL(url);
                const cloudHost = new URL(resolveCloudinaryBaseUrl()).hostname;
                if (u.hostname !== cloudHost) {
                    return res.status(400).json({ ok: false, error: 'Only Cloudinary resources allowed' });
        ...
        app.all('/image/:filename', async (req, res) => {
            try {
                ...
                const CLOUDINARY_CLOUD_NAME = resolveCloudinaryName();
                const url = `${resolveCloudinaryBaseUrl()}/image/upload/${name}`;
                const u = new URL(url);
                const cloudHost = new URL(resolveCloudinaryBaseUrl()).hostname;
                if (u.hostname !== cloudHost) {
                    return res.status(400).json({ ok: false, error: 'Only Cloudinary resources allowed' });
