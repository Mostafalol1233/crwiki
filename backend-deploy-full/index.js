...
function requireSettingsManager(req, res, next) {
    ...
    const hasRole = Array.isArray(roles) && roles.includes("settings_manager");
    if (hasRole || perms["settings:manage"]) return next();
...
function requireContentCreator(req, res, next) {
    ...
    if (isAdmin && (perms["posts:create"] || perms["events:create"] || perms["news:create"] || perms["tutorials:create"])) return next();
...
function requireAdminOnly(req, res, next) {
    ...
    const role = user?.role || "";
    const roles = user?.roles || [];
    const isAdmin = role === "super_admin" || role === "admin" || (Array.isArray(roles) && (roles.includes("super_admin") || roles.includes("admin")));
...
function suggestKeywords(text, limit = 8) {
    const stop = new Set([
        ...
        "من", "على", "في", "عن", "و", "ما", "لا", "لم", "لن", "إلى", "الى", "كان", "كانت", "ذلك", "هذه", "هذا", "إذ", "قد", "لقد", "كما"
    ]);
...
export async function registerRoutes(app) {
    const app2 = app;
    // This is a bridge to the imported registerRoutes
    await registerRoutesImported(app);
...
// server/index-production.ts
    ...
    app2.post("/api/events", requireAuth, requireContentCreator, async (req, res) => {
        try {
            ...
            const withSeo = { ...data };
            withSeo.eventSlug = withSeo.eventSlug || eventSlug;
...
// تأكد أنه لا يوجد قوس إغلاق تائه "}" هنا قبل بداية الـ app2.delete
...
app2.delete("/api/events/:id", requireAuth, requireOwnershipOrAdmin("events"), async (req, res) => {
...
}); // هذا القوس يغلق الـ delete route بشكل صحيح
    ...
    app2.post("/api/tickets", upload.fields([{ name: "image", maxCount: 1 }, { name: "video", maxCount: 1 }]), async (req, res) => {
        try {
            ...
            async function uploadToCloudinaryOrCatbox(file) {
                ...
                if (cloudName && apiKey && apiSecret) {
                    try {
                        ...
                        const json = await cloudinarySignedUpload(file.buffer, file.originalname, file.mimetype, { 
                            folder: "tickets",
                            resource_type: resourceType
                        });
                        
                        if (json.secure_url) {
                            return buildDomainUrl(json.resource_type || resourceType, json.public_id, json.format, req);
                        }
                    } catch (err) {
                        console.warn("[tickets] Cloudinary failed, falling back to local storage:", err.message);
                        // Fallback to local storage using the main upload logic
                        try {
                            const UPLOADS_DIR = path.resolve("uploads");
                            fs.mkdirSync(UPLOADS_DIR, { recursive: true });
                            const baseName = sanitizeFilename(file.originalname || "ticket-upload");
                            ...
                            console.error("[tickets] Local fallback failed:", localErr);
                        }
                    }
                }
            ...
            if (videoFile) {
                mediaUrl = await uploadToCloudinaryOrCatbox(videoFile);
                mediaType = "video";
            } else if (imageFile) {
                mediaUrl = await uploadToCloudinaryOrCatbox(imageFile);
                mediaType = "image";
            }
    ...
    app2.post("/api/upload-image", uploadLimiter, (req, res, next) => {
        ...
        if (!authorized) {
            const csrf = req.headers["x-csrf-token"] || req.headers["x-api-key"] || req.headers["X-CSRF-Token"];
            const envToken = String(process.env.CSRF_TOKEN || process.env.CSRF_SECRET || "").trim();
            if (csrf && envToken && String(csrf).trim() === envToken) {
                authorized = true;
            }
        }

        if (!authorized) {
            return res.status(401).json({ error: "Unauthorized: Missing valid token or CSRF" });
        }
        next();
        ...
        try {
            ...
            // Proactively handle Cloudinary upload if configured
            const cloudName = String(process.env.CLOUDINARY_CLOUD_NAME || "").trim();
            const apiKey = String(process.env.CLOUDINARY_API_KEY || "").trim();
            const apiSecret = String(process.env.CLOUDINARY_API_SECRET || "").trim();
    ...
    app2.get("/api/reviews/seller/:name", async (req, res) => {
        try {
            const seller = await storage.getSellerByName(req.params.name);
    ...
    app2.get("/api/public/settings/announcements", async (_req, res) => {
        try {
            const s = await storage.getSiteSettings();
            res.json({ enabled: !!s.announcementsEnabled });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    });
...
(async () => {
    ...
    // Serve images from MongoDB buckets or Cloudinary proxy
    app.get("/uploads/*", async (req, res) => {
        try {
            const rawPath = req.params[0] || "";
            const filename = rawPath.replace(/\\/g, "/"); // normalize slashes
            if (!filename) return res.status(400).send("Invalid filename");

            // Try Cloudinary first
            const cloudName = String(process.env.CLOUDINARY_CLOUD_NAME || "dkpdidm89").trim();
            // Determine resource type based on extension (simple heuristic)
            const ext = filename.split('.').pop().toLowerCase();
            const type = ['mp4','webm','ogg'].includes(ext) ? 'video' : ['mp3','wav'].includes(ext) ? 'video' : 'image';
            
            // Construct Cloudinary URL - preserve folder structure from filename
            const url = `https://res.cloudinary.com/${cloudName}/${type}/upload/${filename}`;
            const upstream = await fetch(url);
            ...
            // Fallback to local MongoDB storage if not found in Cloudinary
            // Note: MongoDB storage usually doesn't have folders in filename, but let's check
            const cleanName = filename.split("/").pop(); // get just the file name
