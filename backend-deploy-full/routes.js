...
`;
    ...
    app.get("/api/stats", requireAuth, async (req, res) => {
        try {
            const posts = await storage.getAllPosts();
            const allComments = await Promise.all(posts.map((post) => storage.getCommentsByPostId(post.id)));
    ...
    app.post("/api/tickets/:id/replies", upload.single("attachment"), async (req, res) => {
        try {
            ...
            if (req.file) {
                const formData = new global.FormData();
                formData.append("reqtype", "fileupload");
                const blob = new global.Blob([req.file.buffer], { type: req.file.mimetype });
