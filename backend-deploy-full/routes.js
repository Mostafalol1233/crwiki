    ...
    app.post("/api/posts", requireAuth, requirePostManager, async (req, res) => {
        try {
            ...
            try {
                ...
                const seoImage = await generateSeoImage({ baseDir: imagesDir, slug: data.title, title: data.seoTitle || data.title, keywords: data.seoKeywords, type: 'post' });
    ...
    app.post("/api/events", requireAuth, requireEventManager, async (req, res) => {
        try {
            ...
            try {
                const imagesDir = path.resolve('backend-deploy-full/uploads/images');
                fs.mkdirSync(imagesDir, { recursive: true });
                const seoImage = await generateSeoImage({ baseDir: imagesDir, slug: data.title, title: data.seoTitle || data.title, keywords: data.seoKeywords, type: 'event' });
                data.ogImage = seoImage.url;
                data.twitterImage = seoImage.url;
            } catch { }
    ...
    app.delete("/api/mercenaries/:id", requireAuth, requireWeaponManager, async (req, res) => {
        ...
        app.post("/api/news", requireAuth, requireNewsManager, async (req, res) => {
            try {
                ...
                try {
                    const imagesDir = path.resolve('backend-deploy-full/uploads/images');
                    fs.mkdirSync(imagesDir, { recursive: true });
                    const seoImage = await generateSeoImage({ baseDir: imagesDir, slug: data.title, title: data.seoTitle || data.title, keywords: data.seoKeywords, type: 'news' });
                    data.ogImage = seoImage.url;
                    data.twitterImage = seoImage.url;
                } catch { }
        ...
        app.post("/api/tickets", upload.fields([{ name: "image", maxCount: 1 }, { name: "video", maxCount: 1 }]), async (req, res) => {
            try {
                ...
                if (file) {
                    try {
                        mediaUrl = await uploadToCatbox(file.buffer, file.originalname, file.mimetype, { retries: 3, timeoutMs: 12000 });
                    } catch (e) {
                        logUpload({ type: 'catbox_attachment', entity: 'ticket', filename: file.originalname, size: file.size, mimetype: file.mimetype, error: e?.message || String(e) });
                        return res.status(502).json({ error: "Attachment upload failed", code: "catbox_failed" });
                    }
        ...
        app.post("/api/tickets/:id/replies", upload.single("attachment"), async (req, res) => {
            try {
                ...
                if (req.file) {
                    try {
                        mediaUrl = await uploadToCatbox(req.file.buffer, req.file.originalname, req.file.mimetype, { retries: 3, timeoutMs: 12000 });
                    } catch (e) {
                        logUpload({ type: 'catbox_attachment', entity: 'ticket_reply', filename: req.file.originalname, size: req.file.size, mimetype: req.file.mimetype, error: e?.message || String(e) });
                        return res.status(502).json({ error: "Attachment upload failed", code: "catbox_failed" });
                    }
