...
async function seedDatabase(options = {}) {
  ...
  try {
    ...
    // Seed mercenaries (10 total) - only add new ones
    console.log(`\n⚔️ Seeding ${mercenariesData.length} mercenaries...`);
    let mercCount = 0;
    for (const merc of mercenariesData) {
      const exists = await Mercenary.findOne({ name: merc.name });
      if (!exists) {
        await Mercenary.create(merc);
        mercCount++;
      }
    }
    console.log(`  ✅ Seeded: ${mercCount} new mercenaries (${mercenariesData.length - mercCount} already existed)`);

    // Seed weapons - only add new ones
    ...
    console.log(`  ✅ Seeded: ${weaponCount} new weapons (${weaponsData.length - weaponCount} already existed)`);

    // Seed modes - only add new ones
    console.log(`\n🎮 Seeding ${modesData.length} game modes...`);
    let modeCount = 0;
    for (const mode of modesData) {
      const exists = await Mode.findOne({ name: mode.name });
      if (!exists) {
        await Mode.create(mode);
        modeCount++;
      }
    }
    console.log(`  ✅ Seeded: ${modeCount} new modes (${modesData.length - modeCount} already existed)`);

    // Seed ranks - only add new ones
    const finalRanksData = await scrapeRanks();
    console.log(`\n🏅 Seeding ${finalRanksData.length} ranks...`);
    let rankCount = 0;
    for (const rank of finalRanksData) {
      const exists = await Rank.findOne({ name: rank.name });
      if (!exists) {
        await Rank.create(rank);
        rankCount++;
      }
    }
    console.log(`  ✅ Seeded: ${rankCount} new ranks (${finalRanksData.length - rankCount} already existed)`);

    console.log("\n✅ SEEDING COMPLETE!");
    console.log(`   📊 New items added: ${mercCount} mercenaries + ${weaponCount} weapons + ${modeCount} modes + ${rankCount} ranks`);
