...
export default function ScrapingManager() {
  ...
  const handleFetchEvents = async () => {
    ...
    try {
      ...
      const urls = posts.map((post: any) => post.url);
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          setValidation(json);
                          toast({ title: 'Content validated', description: `Found ${json.colors?.length || 0} color styles` });
                        } catch (e: any) {
                  ...
                  {validation && (
                    <div className="mt-3 p-2 rounded border bg-muted">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {validation.colors.map((c) => (
                          <span key={c} className="px-2 py-0.5 rounded text-xs border" style={{ background: '#fff', color: c, borderColor: c }}>{c}</span>
                        ))}
                      </div>
                    </div>
                  )}
