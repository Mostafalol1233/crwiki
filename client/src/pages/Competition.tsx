...
export default function Competition() {
  ...
  return (
    <main dir={direction} style={{ minHeight: "100vh", background: "#090d13", color: "#e5e7eb", paddingBottom: 72 }}>
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "72px 24px 28px" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 20 }}>
          ...
          <span style={{ ...eyebrow, color: "#9ca3af", borderColor: "#374151" }}>{isArabic ? "إنجليزي وعربي" : "English and Arabic"}</span>
        </div>
        <h1 style={{ fontSize: "clamp(38px, 7vw, 78px)", lineHeight: 0.98, letterSpacing: "-0.05em", maxWidth: 850, margin: 0, color: "#f9fafb" }}>{title}</h1>
        <p style={{ maxWidth: 720, color: "#aeb8c4", fontSize: 18, lineHeight: 1.7, marginTop: 24 }}>{intro || (isArabic ? "مسابقة مجتمعية لتقييم معرفة اللاعبين بعالم CrossFire، مع أسئلة عادلة ونظام نقاط قابل للمراجعة." : "A community competition for CrossFire knowledge, with fair questions and an administrator-reviewed scoring policy.")}</p>
...
function QuestionCard({ question, index, isArabic, answer, onAnswer }: { question: CompetitionQuestion; index: number; isArabic: boolean; answer: string; onAnswer: (value: string) => void }) {
  ...
  return <article style={{ border: "1px solid #2b3847", padding: 16, background: "#0b121a", display: "grid", gap: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, color: "#f3f4f6" }}><strong>{index + 1}. {prompt}</strong><small style={{ color: "#94a3b8", whiteSpace: "nowrap" }}>{question.points ?? 0} {isArabic ? "نقطة" : "points"}</small></div>{question.audio_url && <audio controls preload="none" src={question.audio_url} style={{ width: "100%" }} />}{options.length > 0 ? <div style={{ display: "grid", gap: 8 }}>{options.map((option) => <label key={option.value} style={{ display: "flex", gap: 9, alignItems: "flex-start", border: "1px solid #263241", padding: "9px 10px", color: "#cbd5e1", cursor: "pointer" }}><input type="radio" name={`question-${question.id}`} checked={answer === option.value} onChange={() => onAnswer(option.value)} />{option.label}</label>)}</div> : <textarea value={answer} onChange={(event) => onAnswer(event.target.value)} placeholder={isArabic ? "اكتب إجابتك للمراجعة الإدارية" : "Write your answer for administrator review"} style={{ ...input, minHeight: 100, resize: "vertical" }} />}</article>;
}

function Feature({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) { return <article style={{ ...card, display: "grid", gap: 12 }}><div style={{ color: "#cbd5e1" }}>{icon}</div><strong style={{ color: "#f3f4f6" }}>{title}</strong><p style={{ ...muted, margin: 0 }}>{text}</p></article>; }

const panel: React.CSSProperties = { background: "#0e151e", border: "1px solid #253140", padding: 24 };
const card: React.CSSProperties = { background: "#0e151e", border: "1px solid #253140", padding: 20 };
const heading: React.CSSProperties = { color: "#f9fafb", fontSize: 28, margin: 0 };
const muted: React.CSSProperties = { color: "#9ca3af", lineHeight: 1.7, fontSize: 14 };
const eyebrow: React.CSSProperties = { border: "1px solid #64748b", color: "#e2e8f0", padding: "6px 10px", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase" };
const chip: React.CSSProperties = { display: "inline-flex", width: "fit-content", border: "1px solid #3a4654", color: "#cbd5e1", padding: "5px 9px", fontSize: 12 };
const number: React.CSSProperties = { width: 24, height: 24, display: "grid", placeItems: "center", border: "1px solid #64748b", color: "#e2e8f0", flex: "0 0 auto", fontSize: 12 };
const closedBox: React.CSSProperties = { marginTop: 18, border: "1px dashed #475569", color: "#aeb8c4", padding: 16, lineHeight: 1.6, fontSize: 13 };
const label: React.CSSProperties = { display: "grid", gap: 7, color: "#cbd5e1", fontSize: 13 };
const input: React.CSSProperties = { background: "#090d13", border: "1px solid #334155", color: "#f8fafc", padding: "11px 12px", outline: "none", width: "100%", boxSizing: "border-box" };
const button: React.CSSProperties = { background: "#d1d5db", color: "#111827", border: 0, padding: "12px 14px", cursor: "pointer", fontWeight: 700 };
const scoreBox: React.CSSProperties = { display: "flex", gap: 10, alignItems: "center", border: "1px solid #64748b", padding: 16, color: "#f3f4f6" };
