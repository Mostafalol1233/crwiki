...
export default function Home() {
  ...
  const mostViewed = useMemo(() => {
    if (!isAdmin) return [] as { id: string; post_slug?: string; title: string; views: number }[];
    return [...allPosts]
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen bg-gradient-to-b from-background via-background to-background/80">
        ...
        <HeroSection post={heroPost} isPlaceholder={!hasFeaturedPost} bgImageUrl={heroBgUrl} />
            ...
            <>
              ...
              {(allNews.length > 0 || allEvents.length > 0 || allPosts.length > 0) && (
                <section className="mb-12 md:mb-16">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                    ...
                    {allPosts[0] && (
                      <Link href={`/posts/${(allPosts[0] as any).post_slug || allPosts[0].id}`} className="block" data-testid="home-latest-post">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-12">
                ...
                {allPosts.filter((p: any) => p.featured && p.previewOnHome !== false).slice(0, 2).map((post: any) => (
                  <Link key={post.id} href={`/posts/${(post as any).post_slug || post.id}`} className="block">
                    <div className="cursor-pointer h-full overflow-hidden group">
                          ...
                          <Link key={tutorial.id} href={`/videos/${tutorial.category || 'general'}`} className="block group">
                            <div className="relative aspect-video overflow-hidden rounded-xl bg-muted">
                              {((tutorial as any).thumbnail || (tutorial as any).image) ? (
                                <img src={(tutorial as any).thumbnail || (tutorial as any).image} alt={tutorial.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                              ) : (
                            ...
                            <h3 className="mt-3 font-semibold line-clamp-2 group-hover:text-primary transition-colors">
                              {tutorial.title}
                            </h3>
                          </Link>
                        ))}
                  ...
                  {mostViewed.length > 0 && (
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-2">
                        {mostViewed.map((post) => (
                          <li key={post.id}>
                            <Link href={`/posts/${(post as any).post_slug || post.id}`} className="text-sm hover:text-primary transition-colors">
                    ...
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-3">
                        {recentPosts.map((post) => (
                          <li key={post.id}>
                            <Link href={`/posts/${(post as any).post_slug || post.id}`} className="block group">
