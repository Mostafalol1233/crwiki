      ...
      <div className="min-h-screen">
        <div className="max-w-[1600px] mx-auto px-3 sm:px-4 md:px-8">
          ...
          <div className="py-8 md:py-12 space-y-12 md:space-y-16">
            ...
            <section className="space-y-6 md:space-y-10">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                {allEvents.filter(e => !e.rawHtmlContent).slice(0, 4).map((event: any) => (
                  <Link key={event.id} href={event.event_name_slug ? `/events/${event.event_name_slug}` : `/events/${event.id}`}>
                    <Card className="group relative overflow-hidden aspect-[3/4] md:aspect-[4/5] border-0 rounded-lg md:rounded-none cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-300">
                      <img 
                      ...
                      <div className="absolute bottom-0 left-0 p-3 md:p-6 space-y-1 md:space-y-2 w-full">
                        <h3 className="text-white font-bold text-xs sm:text-sm md:text-xl uppercase tracking-tight line-clamp-2">
                          {event.title}
                        </h3>
                        <p className="text-white/70 text-[9px] sm:text-[10px] md:text-xs font-bold uppercase tracking-widest">
                          {event.date}
                        </p>
                      </div>
                    </Card>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
                {allEvents.filter(e => !e.rawHtmlContent).slice(4, 7).map((event: any) => (
                  <Link key={event.id} href={event.event_name_slug ? `/events/${event.event_name_slug}` : `/events/${event.id}`}>
                    <Card className="group relative overflow-hidden aspect-video border-0 rounded-lg md:rounded-none cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-300">
                      <img 
                        src={event.image || 'https://files.catbox.moe/wof38b.jpeg'} 
                        alt={event.title} 
                        className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
            ...
            <div className="wiki-content-card rounded-xl md:rounded-2xl overflow-hidden">
