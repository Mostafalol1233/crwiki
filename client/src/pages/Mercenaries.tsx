...
import { Volume2, VolumeX, Grid3x3, Zap, StopCircle, Users, Swords } from "lucide-react";
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex rounded overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
                <button
                  ...
                  style={{
                    background: layoutStyle === "strip" ? "#f5a623" : "var(--card)",
                    color: layoutStyle === "strip" ? "#000" : "#666",
                  }}
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 text-[10px] font-black uppercase tracking-wider transition-all"
                  style={{
                    background: layoutStyle === "grid" ? "#f5a623" : "var(--card)",
                    color: layoutStyle === "grid" ? "#000" : "#666",
              ...
              <button
                ...
                style={{
                  background: stopFlash ? "#ef4444" : "var(--card)",
                  color: stopFlash ? "#fff" : "#666",
                  border: `1px solid ${stopFlash ? "#ef4444" : "rgba(255,255,255,0.08)"}`,
                }}
              >
              ...
              {mercenaries.map((merc) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      width: isExpanded ? "340px" : "88px",
                      minWidth: isExpanded ? "340px" : "88px",
                  ...
                  >
                    ...
                    {isExpanded && <div className="absolute inset-0" style={{ background: `linear-gradient(to right, transparent 50%, rgba(0,0,0,0.3) 100%)` }} />}

                    {/* Top accent on expand */}
                    {isExpanded && (
                      <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: "linear-gradient(to right, #f5a623, transparent)" }} />
                    )}

                    {/* Collapsed name (rotated) */}
                    {!isExpanded && (
                      <div
                        className="absolute bottom-6 left-0 right-0 flex items-center justify-center"
                        style={{ transform: "rotate(-90deg) translateX(-30%)", transformOrigin: "center" }}
                      >
                        <span className="text-[9px] font-black uppercase tracking-[0.2em] whitespace-nowrap" style={{ color: "rgba(255,255,255,0.6)" }}>
                          {merc.name}
                        </span>
                      </div>
                    )}
                      ...
                      <div className="absolute inset-0 flex flex-col justify-end p-5 z-10">
                        ...
                        <div className="mb-3">
                          <span
                            ...
                            style={{ background: roleStyle.bg, color: roleStyle.color, borderRadius: "2px" }}
                          >
                            {merc.role}
                          </span>
                        </div>

                        <h3 className="text-white text-2xl font-black uppercase tracking-tight leading-tight mb-1">
                          {merc.name}
                        </h3>
                        ...
                        {merc.voiceLines && merc.voiceLines.length > 0 ? (
                          <button
                            onClick={(e) => { e.stopPropagation(); playRandomSound(merc.id, merc.voiceLines); }}
                            ...
                            style={{
                              background: playingMercId === merc.id ? "rgba(239,68,68,0.8)" : "rgba(245,166,35,0.9)",
                              color: "#000",
                              borderRadius: "2px",
                            }}
                          >
                            {playingMercId === merc.id ? (
                              <><VolumeX className="h-3.5 w-3.5" /> Playing... ({merc.voiceLines.length} lines)</>
                            ) : (
                              <><Volume2 className="h-3.5 w-3.5" /> Play Voice Line ({merc.voiceLines.length})</>
                            )}
                          </button>
                        ) : (
                ...
                return (
                  ...
                  >
                    ...
                    {merc.voiceLines && merc.voiceLines.length > 0 && (
                      <div className="absolute top-2.5 right-2.5">
                        <div
                          ...
                          style={{ background: "rgba(0,0,0,0.7)", borderRadius: "2px", border: "1px solid rgba(245,166,35,0.3)" }}
                        >
                          <Volume2 className="h-2.5 w-2.5" style={{ color: "#f5a623" }} />
                          <span className="text-[8px] font-bold" style={{ color: "#f5a623" }}>{merc.voiceLines.length}</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3 z-10">
                      <span
                        className="text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 mb-1.5 inline-block"
                        style={{ background: roleStyle.bg, color: roleStyle.color, borderRadius: "2px" }}
                      >
                        {merc.role}
                      </span>
        ...
        >
          ...
          >
            ...
            <div className="relative h-72 overflow-hidden">
              <img
                src={resolveMercImage(selectedMerc)}
                alt={selectedMerc.name}
            ...
            <div className="px-6 pb-6">
              ...
              {selectedMerc.voiceLines && selectedMerc.voiceLines.length > 0 && (
                <button
                  onClick={() => playRandomSound(selectedMerc.id, selectedMerc.voiceLines)}
                  className="w-full flex items-center justify-center gap-2 py-3 text-[11px] font-black uppercase tracking-widest transition-all hover:brightness-110"
                ...
                >
                  {playingMercId === selectedMerc.id ? (
                    ...
                    <><Volume2 className="h-4 w-4" /> Play Random Voice Line ({selectedMerc.voiceLines.length})</>
                  )}
                </button>
              )}
            </div>
