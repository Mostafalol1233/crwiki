...
const RANK_EXP: Record<number, { name: string; exp: number }> = {
  ...
  // ── BG through Grand Marshall — EXP from formula: round(487 × t^2.5, −3)
  // Calibrated against real player data: 28.18M EXP places user above BG5 (27.88M)
  // and below BG6 (28.76M), giving correct sub-rank detection and EXP-to-go display.
  76:  { name: "Brigadier General 1",    exp: 24_524_000 },
  77:  { name: "Brigadier General 2",    exp: 25_335_000 },
  78:  { name: "Brigadier General 3",    exp: 26_164_000 },
  79:  { name: "Brigadier General 4",    exp: 27_015_000 },
  80:  { name: "Brigadier General 5",    exp: 27_878_000 },
  81:  { name: "Brigadier General 6",    exp: 28_757_000 },
  82:  { name: "Major General 1",        exp: 29_649_000 },
  83:  { name: "Major General 2",        exp: 30_564_000 },
  84:  { name: "Major General 3",        exp: 31_495_000 },
  85:  { name: "Major General 4",        exp: 32_440_000 },
  86:  { name: "Major General 5",        exp: 33_398_000 },
  87:  { name: "Major General 6",        exp: 34_371_000 },
  88:  { name: "Lieutenant General 1",   exp: 35_358_000 },
  89:  { name: "Lieutenant General 2",   exp: 36_383_000 },
  90:  { name: "Lieutenant General 3",   exp: 37_443_000 },
  91:  { name: "Lieutenant General 4",   exp: 38_486_000 },
  92:  { name: "Lieutenant General 5",   exp: 39_541_000 },
  93:  { name: "Lieutenant General 6",   exp: 40_618_000 },
  94:  { name: "General 1",              exp: 41_723_000 },
  95:  { name: "General 2",              exp: 42_856_000 },
  96:  { name: "General 3",              exp: 43_982_000 },
  97:  { name: "General 4",              exp: 45_131_000 },
  98:  { name: "General 5",              exp: 46_298_000 },
  99:  { name: "General 6",              exp: 47_490_000 },
  100: { name: "General 7",              exp: 48_700_000 },
  101: { name: "General 8",              exp: 49_928_000 },
  102: { name: "General 9",              exp: 51_172_000 },
  103: { name: "General 10",             exp: 52_435_000 },
  104: { name: "Grand Marshall",         exp: 53_717_000 },
};
...
export default function Profile() {
  ...
  const handleGetTips = async () => {
    ...
    setTipsError("");
    try {
      const currentRank = (cfStats.rank || rp.curInfo.name || "").toLowerCase();
      const targetRank = rp.destInfo?.name ?? "next rank";
      const kd = cfStats.kdRatio ?? null;
      const wr = cfStats.winRate ?? null;
      const clan = cfStats.clan ?? null;
      const vipLevel = cfStats.vipLevel ?? null;

      const isLowRank  = /trainee|private|corporal|sergeant/.test(currentRank);
      const isMidRank  = /lieutenant|captain|major/.test(currentRank);
      const isHighRank = /colonel|brigadier|general/.test(currentRank);
      const isElite    = /marshall|marshal/.test(currentRank);
      ...
      if (vipLevel != null) {
        ...
        tips.push(`At this rank the EXP gaps between sub-tiers are large. Use weapon crates and daily missions to supplement game EXP — every extra source counts when you need millions of EXP per tier.`);
      } else {
