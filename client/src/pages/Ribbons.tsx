...
function isRecurring(ribbon: Ribbon) {
  ...
  return value.includes("recurr") || value.includes("annual") || value.includes("seasonal");
}
...
function RibbonCard({ ribbon, arabic, onOpen }: { ribbon: Ribbon; arabic: boolean; onOpen: () => void }) {
  const unlocked = Boolean(ribbon.unlocked);
  ...
  return <article className="group overflow-hidden border border-white/10 bg-[#101722] transition hover:border-amber-400/45 hover:bg-[#131c28]">
    ...
    <div className="space-y-3 p-4">
      <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-400">{label(ribbon.source_category, arabic)}</p><h2 className="mt-1 text-lg font-black text-white">{name}</h2>{ribbon.name_ar && <p className="mt-1 text-sm text-slate-400" dir="rtl">{ribbon.name_ar}</p>}</div><StateBadge tone={unlocked ? "good" : "neutral"}>{unlocked ? (arabic ? "مفتوحة" : "Unlocked") : (arabic ? "مقفلة" : "Locked")}</StateBadge></div>
      <div className="flex flex-wrap gap-1.5"><StateBadge>{label(derivedDifficulty(ribbon), arabic)}</StateBadge>{ribbon.paid_requirement && ribbon.paid_requirement !== "none" && <StateBadge tone="warning">{arabic ? "تتطلب شراء أو حالة" : "Paid / status"}</StateBadge>}{isRecurring(ribbon) && <StateBadge tone="warning">{arabic ? "عيلة إيفنت متكررة" : "Recurring family"}</StateBadge>}</div>
...
function RibbonDetails({ ribbon, arabic }: { ribbon: Ribbon; arabic: boolean }) {
  ...
  return <div className="space-y-6 text-white">
    <div className="grid gap-5 md:grid-cols-[220px_1fr]"><RibbonImage ribbon={ribbon} large /><div className="space-y-3"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-400">{label(ribbon.source_category, arabic)}</p><h2 className="mt-2 text-3xl font-black">{name}</h2>{ribbon.name_ar && <p className="mt-2 text-base text-slate-300" dir="rtl">{ribbon.name_ar}</p>}</div><div className="flex flex-wrap gap-2"><StateBadge tone={ribbon.unlocked ? "good" : "neutral"}>{ribbon.unlocked ? "Unlocked" : "Locked"}</StateBadge><StateBadge>{label(ribbon.exactness, arabic)}</StateBadge><StateBadge>{text(ribbon.ownership_count_display || ribbon.ownership_count, "—")} ownership</StateBadge></div><dl className="grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Ribbon ID</dt><dd className="mt-1 font-bold text-white">{text(ribbon.ribbon_id, "—")}</dd></div><div><dt className="text-slate-500">Difficulty</dt><dd className="mt-1 font-bold text-white">{label(derivedDifficulty(ribbon), arabic)}</dd></div></dl></div></div>
...
export default function Ribbons() {
  ...
  const filtered = useMemo(() => {
    ...
    const rows = ribbons.filter((ribbon) => {
      ...
      if (selectedStatus === "unlocked" && !ribbon.unlocked) return false;
      if (selectedStatus === "locked" && ribbon.unlocked) return false;
    ...
    return [...rows].sort((a, b) => {
      if (sort === "unlocked") return Number(Boolean(b.unlocked)) - Number(Boolean(a.unlocked));
      if (sort === "locked") return Number(Boolean(a.unlocked)) - Number(Boolean(b.unlocked));
  ...
  }, [query, selectedCategory, selectedStatus, selectedAvailability, selectedEvent, selectedRecurrence, selectedDifficulty, paidOnly, knownHistory, exactDates, sort]);

  const unlocked = ribbons.filter((r) => r.unlocked).length;
  const recurring = ribbons.filter(isRecurring).length;
  const documented = ribbons.filter(hasReleaseHistory).length;
  const resetFilters = () => { setQuery(""); setSelectedCategory("all"); setSelectedStatus("all"); setSelectedAvailability("all"); setSelectedEvent("all"); setSelectedRecurrence("all"); setSelectedDifficulty("all"); setPaidOnly(false); setKnownHistory("all"); setExactDates("all"); setSort("name"); };
