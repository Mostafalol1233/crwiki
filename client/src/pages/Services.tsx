...
const localPath = (path: string, _language: "en" | "ar") => routerPath(path);

function contactHref(key: string, value: string) {
  const raw = String(value || "").trim();
  if (!raw) return "";
  if (key === "email") return raw.startsWith("mailto:") ? raw : `mailto:${raw}`;
  if (key === "phone") return raw.startsWith("tel:") ? raw : `tel:${raw}`;
  try {
...
function ServiceImage({ src, alt, className, fallback }: { src: string; alt: string; className: string; fallback: string }) {
  const [failed, setFailed] = useState(false);
  if (!src || failed) {
    return <div className={`${className} flex items-center justify-center bg-muted/70 text-center text-xs font-bold text-muted-foreground`} role="img" aria-label={alt}>{fallback}</div>;
  }
      ...
      <main className="min-h-screen bg-background">
        ...
        <section className="mx-auto max-w-6xl px-4 py-8 md:px-8 md:py-12">
          ...
          {filteredListings.length === 0 ? <Card><CardContent className="py-12 text-center text-muted-foreground">{isArabic ? "لا توجد نتائج مطابقة." : "No matching listings found."}</CardContent></Card> : <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{filteredListings.map((listing) => {
            ...
            const profileHref = contactHref("website", listing.profileUrl || listing.contacts?.funpay || "");
            ...
            return <Card key={`${listing.id || listing.seller}-${listing.profileUrl}`} className="group flex h-full flex-col overflow-hidden border-border/60 bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
              <div className={`relative overflow-hidden bg-gradient-to-br ${brand.accent} px-5 pb-5 pt-4}`}><div className="absolute -right-10 -top-12 h-32 w-32 rounded-full bg-white/25 blur-2xl" />{mediaHero && <div className="relative mb-4 flex min-h-40 items-center justify-center overflow-hidden rounded-2xl border border-white/50 bg-slate-950/10 p-3 shadow-inner"><ServiceImage src={mediaHero} alt={`${listing.seller} CrossFire service`} fallback={initials} className={heroIsProfile ? "h-28 w-28 rounded-full object-cover ring-2 ring-white/30" : "max-h-44 w-full object-contain"} /></div>}<div className="relative flex items-center gap-4"><div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-white/70 bg-white/90 p-2 shadow-lg dark:bg-slate-950/80">{mediaLogo ? <ServiceImage src={mediaLogo} alt={`${listing.seller} logo`} fallback={initials} className="h-full w-full object-contain" /> : <span className="text-xl font-black text-primary">{initials}</span>}</div><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-foreground/60">{isArabic ? "متجر الخدمة" : "Service storefront"}</p><h3 className="truncate text-xl font-black tracking-tight">{listing.seller}</h3><Badge variant="outline" className="mt-2 border-foreground/15 bg-background/60 text-[10px]">{isArabic ? "الملف متاح" : "Profile available"}</Badge></div></div>{mediaGallery.length > 1 && <div className="relative mt-4 grid grid-cols-3 gap-2">{mediaGallery.slice(1, 4).map((image, idx) => <div key={`${image}-${idx}`} className="aspect-[4/3] overflow-hidden rounded-lg border border-white/40 bg-black/10 p-1"><ServiceImage src={image} alt={`${listing.seller} CrossFire service artwork ${idx + 2}`} fallback={initials} className="h-full w-full object-contain" /></div>)}</div>}</div>
              ...
              <CardContent className="flex-grow space-y-3 text-sm"><div className="flex gap-2 rounded-xl border bg-muted/40 p-3"><Tag className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span>{isArabic && listing.priceAr ? listing.priceAr : listing.price}</span></div><div className="flex gap-2 text-xs text-muted-foreground"><Clock3 className="h-4 w-4 shrink-0" /><span>{isArabic && listing.ageAr ? listing.ageAr : listing.age}</span></div><p className="text-xs leading-5 text-muted-foreground">{isArabic ? `الصورة المعروضة مأخوذة من الملف العام للبائع، وليست صورة عشوائية من تصنيفات CrossFire. لا توجد صورة مستقلة للعرض موثقة حالياً؛ راجع العرض الأصلي قبل التواصل.` : `The image shown comes from the public seller profile, not random CrossFire category artwork. No separate service image is currently verified; review the original listing before contacting the seller.`}</p><div className="flex flex-wrap gap-2 pt-1">{contacts.map((contact) => { const href = contactHref(contact.key, contact.value); return href ? <a key={`${contact.key}-${contact.value}`} href={href} target={contact.key === "email" || contact.key === "phone" ? undefined : "_blank"} rel={contact.key === "email" || contact.key === "phone" ? undefined : "noreferrer"} className="inline-flex items-center gap-1.5 rounded-full border bg-background px-2.5 py-1 text-[11px] font-medium text-muted-foreground transition-colors hover:border-primary/40 hover:text-primary">{contactIcon(contact.key)}{isArabic ? AR_CONTACT_LABELS[contact.key] || contact.label : contact.label}</a> : null; })}</div></CardContent>
              <CardFooter className="border-t bg-muted/20 p-4"><Button asChild={Boolean(profileHref)} className="w-full" size="sm" disabled={!profileHref}>{profileHref ? <a href={profileHref} target="_blank" rel="noreferrer"><UserRound className="mr-2 h-4 w-4" />{isArabic ? "فتح الملف الشخصي" : "Open seller profile"}<ExternalLink className="ml-auto h-3.5 w-3.5" /></a> : <span><UserRound className="mr-2 inline h-4 w-4" />{isArabic ? "لا يوجد رابط ملف موثق" : "No verified profile link"}</span>}</Button></CardFooter>
            </Card>;
          })}</div>}
        </section>
