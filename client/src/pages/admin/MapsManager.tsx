...
const EMPTY: Partial<GameMap> = { name: '', mode: '', category: 'Official', image_url: '', description: '' };
const col = createColumnHelper<GameMap>();

export default function MapsManager() {
  const [view, setView] = useState<'list' | 'form'>('list');
  const [items, setItems] = useState<GameMap[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [editing, setEditing] = useState<Partial<GameMap>>(EMPTY);
  const client = supabaseService;

  const fetch = useCallback(async () => {
    if (!client) return;
    setLoading(true);
    const { data } = await client.from('maps').select('*').order('name');
    setItems(data || []);
    setLoading(false);
  }, [client]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (editing.id) { await client.from('maps').update(editing).eq('id', editing.id); toast.success('Updated'); }
      else { await client.from('maps').insert(editing); toast.success('Created'); }
  ...
  const columns = [
    ...
    col.accessor('mode', { header: 'Mode', cell: (i) => <span style={{ fontSize: 12, color: '#a1a1aa' }}>{i.getValue() || '—'}</span> }),
    col.accessor('category', { header: 'Category', cell: (i) => <span style={{ fontSize: 12, color: '#52525b' }}>{i.getValue()}</span> }),
    col.display({
      ...
      cell: (i) => (
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button type="button" onClick={async () => { if (client && confirm('Delete?')) { await client.from('maps').delete().eq('id', i.row.original.id); toast.success('Deleted'); await fetch(); } }} style={{ padding: '4px 10px', background: 'transparent', border: '1px solid #27272a', borderRadius: 4, color: '#ef4444', cursor: 'pointer' }}><Trash2 size={12} /></button>
        </div>
      ),
    }),
  ];
    ...
    return (
      <div style={{ maxWidth: 700 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <h1 style={{ fontSize: 18, fontWeight: 600, color: '#fafafa', margin: 0 }}>{editing.id ? 'Edit Map' : 'New Map'}</h1>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            ...
            <div><label style={lbl}>Mode</label><input type="text" value={editing.mode || ''} onChange={(e) => setEditing({ ...editing, mode: e.target.value })} style={inp} /></div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 1000 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h1 style={{ fontSize: 20, fontWeight: 600, color: '#fafafa', margin: 0 }}>Maps</h1>
        <button type="button" onClick={() => { setEditing(EMPTY); setView('form'); }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', background: '#d4a017', border: 'none', borderRadius: 4, color: '#09090b', fontWeight: 500, cursor: 'pointer', fontSize: 13 }}><Plus size={14} />New Map</button>
      </div>
      <DataTable data={items} columns={columns} loading={loading} searchPlaceholder="Search maps..." />
    </div>
  );
}
