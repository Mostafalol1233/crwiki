...
export interface IStorage {
  ...
  removeDuplicateMercenaries(): Promise<number>;
...
export class MongoDBStorage implements IStorage {
  ...
  async removeDuplicateMercenaries(): Promise<number> {
    await this.connect();
    const all = await MercenaryModel.find().lean();
    ...
    for (const merc of all) {
      ...
      if (seen.has(key)) {
        toDelete.push(merc._id.toString());
      } else {
        seen.set(key, merc._id.toString());
      }
    }
